...
def description(): return "Set the prefix for the bot"
...
async def execute(msg, args, client):
    try:
        if len(args) == 0 or len(args[0]) > 2:
            await msg.reply(embed = denied_msg("Invalid Prefix", ""))
            return

        db_settings.set_prefix(msg.guild, args[0])

        await msg.channel.send(embed = granted_msg("Prefix Changed Successfully"))
